    ...
    <div className="border border-border/60 bg-card/60 rounded-lg p-3 font-mono text-xs space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-primary">
          ...
          <span className="font-bold uppercase tracking-wider text-[11px]">{t('Data Provenance & Origin')}</span>
        </div>
        <button
          onClick={() => setExpanded(!expanded)}
          className="text-[10px] text-muted-foreground hover:text-foreground underline"
        >
          {expanded ? t('Collapse') : t('Inspect Provenance')}
        </button>
      </div>

      <div className="flex items-center gap-3 text-muted-foreground text-[11px]">
        <div>{t('Source:')} <span className="text-foreground font-semibold">{provenance.source_name} ({provenance.source_type})</span></div>
        <div>{t('Collected:')} <span className="text-foreground">{new Date(provenance.collected_at).toLocaleDateString()}</span></div>
      </div>
      ...
      {expanded && (
        <div className="pt-2 border-t border-border/40 space-y-2 text-[11px] animate-in fade-in">
          <div className="p-2 bg-muted/40 rounded border border-border/40 space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">{t('Source Identifier:')}</span>
              <span className="font-bold text-foreground">{provenance.source_identifier}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">{t('Collection Method:')}</span>
              <span className="text-foreground">{provenance.collection_method}</span>
            </div>
            {provenance.source_url && (
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">{t('Source URL / Ref:')}</span>
          ...
          <div className="p-2 bg-primary/10 rounded border border-primary/30 flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-primary">
              ...
              <span className="font-bold">{t('SHA-256 Integrity Hash:')}</span>
            </div>
          ...
          <div className="flex items-center gap-1.5 text-[10px] text-emerald-400 bg-emerald-500/10 p-1.5 rounded border border-emerald-500/30">
            ...
            <span>{t('Original raw record preserved. AI processing is cleanly separated from source intelligence.')}</span>
          </div>
        </div>
      )}
    </div>
