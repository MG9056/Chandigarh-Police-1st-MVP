  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto h-full flex flex-col">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">{t('Perform advanced searches across aliases, wallet addresses, and keywords.')}</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-2 mb-8">
        ...
        <Button type="button" variant="outline" className="h-[50px] px-4"><Filter className="w-5 h-5 mr-2"/> {t('Filters')}</Button>
      ...
      <div className="flex-1 flex flex-col">
        {results.length > 0 ? (
          <div className="space-y-3">
            <h3 className="font-semibold text-lg mb-3">{t('Intelligence Results')} ({results.length})</h3>
            {results.map((res, i) => (
              <div key={i} className="p-4 rounded-xl border bg-card hover:bg-muted/30 transition-colors cursor-pointer flex justify-between items-center">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-xs uppercase tracking-wider text-muted-foreground font-semibold">{t(res.type) || res.type}</span>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">{t('Risk Score')}:</span>
                    <span className={`text-sm font-bold ${res.risk_score > 85 ? 'text-red-500' : 'text-yellow-500'}`}>{res.risk_score}</span>
                  </div>
                  <span className="text-xs text-muted-foreground">{t('Last seen')}: {res.last_seen}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground border-2 border-dashed border-border rounded-xl">
            ...
            <p>{t('Enter a query to search intelligence records.')}</p>
            <p className="text-sm mt-1 opacity-70">{t('Historical records are automatically maintained for audit purposes.')}</p>
          </div>
        )}
      </div>
    </div>
