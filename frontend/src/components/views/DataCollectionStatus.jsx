...
export default function DataCollectionStatus() {
  ...
  const fetchActivity = async () => {
    try {
      const res = await apiFetch(
        '/api/crawler/activity?page=1&page_size=20'
      );
  ...
  const handleTriggerRun = async (sourceId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setActionError(
          errData.detail || 'Failed to trigger crawler run'
        );
      }
    } catch (err) {
      console.error('Error triggering crawl run:', err);
      setActionError(
        'Network error while triggering crawler run'
      );
    } finally {
  ...
  const handleStopSource = async (sourceId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errData = await res.json().catch(() => ({}));
        setActionError(
          errData.detail || 'Failed to stop crawler target'
        );
      }
    } catch (err) {
    ...
    triggerReAuth(async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const errData = await res.json().catch(() => ({}));
          setActionError(
            errData.detail || 'Failed to delete crawler target'
          );
        }
      } catch (err) {
  ...
  const handleToggleSource = async (source) => {
    try {
      ...
      console.error(
        'Error toggling source active state:',
        err
      );
    }
  };
  ...
  const handleAddSourceSubmit = async (e) => {
    ...
    try {
      ...
      if (newSource.source_type === 'POLICE_API') {
        if (!newSource.api_url.trim()) {
          setActionError('Police API URL is required.');
          return;
        }

        config = {
          url: newSource.api_url.trim(),
          method: 'GET',
        };
      } else {
        const urlsArray = newSource.seed_urls
          .split('\n')
          .map((u) => u.trim())
          .filter(Boolean);
      ...
      const res = await apiFetch('/api/sources', {
        ...
        body: JSON.stringify({
          ...
          poll_interval_seconds: parseInt(
            newSource.poll_interval_seconds
          ),
          crawl_delay_seconds: parseFloat(
            newSource.crawl_delay_seconds
          ),
      ...
      if (res.ok) {
        ...
        setActionError(
          errData.detail || 'Failed to create target'
        );
      }
    } catch (err) {
  ...
  const openEditModal = (src) => {
    const isPoliceApi = src.source_type === 'POLICE_API';

    const seedUrlsText = Array.isArray(
      src.config?.seed_urls
    )
      ? src.config.seed_urls.join('\n')
      : '';

    const apiUrl = isPoliceApi
      ? src.config?.url || ''
      : '';
    ...
    setEditingSource({
      ...
      poll_interval_seconds:
        src.poll_interval_seconds || 60,
      crawl_delay_seconds:
        src.crawl_delay_seconds || 1.0,
      transport_type:
        src.transport_type || 'direct',
  ...
  const handleEditSourceSubmit = (e) => {
    ...
    triggerReAuth(async () => {
      ...
      try {
        ...
        if (
          editingSource.source_type === 'POLICE_API'
        ) {
          if (!editingSource.api_url.trim()) {
            setActionError(
              'Police API URL is required.'
            );
            return;
          }

          config = {
            url: editingSource.api_url.trim(),
            method: 'GET',
          };
        } else {
          const urlsArray = editingSource.seed_urls
            .split('\n')
            .map((u) => u.trim())
            .filter(Boolean);
        ...
        const res = await apiFetch(
          `/api/sources/${editingSource.id}`,
          {
            ...
            body: JSON.stringify({
              ...
              config,
              poll_interval_seconds: parseInt(
                editingSource.poll_interval_seconds
              ),
              crawl_delay_seconds: parseFloat(
                editingSource.crawl_delay_seconds
              ),
              transport_type:
                editingSource.transport_type,
        ...
        if (res.ok) {
          ...
          setActionError(
            errData.detail || 'Failed to update target'
          );
        }
      } catch (err) {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto h-full font-mono text-slate-100">
      ...
      <div className="mb-6 flex flex-wrap justify-between items-center gap-4 border-b border-border/40 pb-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight uppercase text-foreground mb-1">
            {t(
              'Multi-Source Data Collection & Crawler Control'
            )}
          </h2>

          <p className="text-muted-foreground text-xs">
            {t(
              'Monitor, configure, and control automated intelligence aggregation nodes.'
            )}
          </p>
        </div>
      ...
      <div className="flex border-b border-border/40 mb-6 gap-2 overflow-x-auto">
        ...
        >
          ...
          {t('Watchlists & Keywords')} (
          {keywordsData.global_keywords.length})
        </button>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-0.5">
                      ...
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase ${
                          src.transport_type ===
                            'tor_proxy' ||
                          src.source_type === 'TOR_STUB'
                            ? 'bg-purple-950/80 text-purple-300 border border-purple-800/50'
                            : 'bg-muted text-muted-foreground'
                        }`}
                      >
                        {src.source_type === 'TOR_STUB'
                          ? 'Tor Stub (Demo Mode Enforced)'
                          : src.transport_type ===
                            'tor_proxy'
                          ? 'Tor Proxy'
                          : 'Direct HTTP'}
              ...
              <div className="grid grid-cols-2 gap-2 text-xs bg-muted/60 p-2.5 rounded-lg border border-border/80 mb-4">
                ...
                <div>
                  ...
                  <span className="text-foreground font-medium">
                    {src.poll_interval_seconds} secs
                  </span>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-xl border border-border bg-card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {keywordsData.global_keywords.map(
                (kw) => (
                  <div
                    key={kw.id}
                    className="p-3 rounded-lg bg-background border border-border flex justify-between items-center"
                  >
                    <div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {activity.items.map((run) => (
              <div
                key={run.id}
                className="p-4 rounded-xl border border-border bg-card hover:bg-muted/40 transition-colors"
              >
          ...
          <div className="bg-slate-900 border border-emerald-500/50 rounded-xl max-w-3xl w-full max-h-[85vh] overflow-y-auto p-6 font-mono shadow-2xl relative animate-in fade-in zoom-in-95">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <span className="text-slate-300">
                    {selectedRecord.relevance_reasoning ||
                      'N/A'}
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRecord.extracted_candidates?.map(
                    (cand, i) => (
                      <span
                        key={i}
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <option value="GOOGLE_SEARCH_DISCOVERY">
                    GOOGLE_SEARCH_DISCOVERY (Tavily API)
                  </option>

                  <option value="BITCOIN_CHAIN">
                    BITCOIN_CHAIN (Public Ledger)
                  </option>
                  ...
                  <option value="TOR_STUB">
                    TOR_STUB (Architecture Stub)
                  </option>

                  <option value="POLICE_API">
                    POLICE_API (Police JSON API)
                  </option>
                </select>
              </div>
              ...
              'POLICE_API' ? (
                <div>
                  ...
                  <input
                    type="url"
                    required
                    placeholder="https://example.gov/api/incidents"
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            ...
            <div className="bg-slate-900 border border-emerald-500/50 rounded-xl max-w-lg w-full p-6 font-mono shadow-2xl relative">
              ...
              <p className="text-[11px] text-amber-400 mb-4 flex items-center gap-1">
                ...
                Sensitive Operation: Password Re-Authentication will be required on submit.
              </p>
