...
export default function AccessControl() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto h-full flex flex-col gap-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight mb-2">{t('Security & Access Control')}</h2>
        <p className="text-muted-foreground">{t('Manage roles, review audit logs, and ensure protection of sensitive investigative information.')}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-xl border bg-card shadow-sm flex flex-col items-center text-center">
          ...
          <h3 className="text-xl font-bold">{t('Investigator Session')}</h3>
          <p className="text-muted-foreground mb-1">{t('Clearance Level')}: {t('Top Secret (Tier 1)')}</p>
          <p className="text-sm text-green-500 font-mono mb-6">{t('Connection')}: {t('Secure (256-bit AES)')}</p>
        ...
        <div className="space-y-4 flex flex-col">
          <div className="p-5 rounded-xl border bg-card hover:bg-muted/20 transition-colors flex items-center justify-between cursor-pointer">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('Manage investigator access levels.')}</p>
              </div>
            </div>
          ...
          <div className="p-5 rounded-xl border bg-card hover:bg-muted/20 transition-colors flex items-center justify-between cursor-pointer">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="font-semibold">{t('Audit Logs')}</h4>
                <p className="text-sm text-muted-foreground">{t('Review system access and search history.')}</p>
              </div>
            </div>
            <Button variant="outline" size="sm">{t('View Logs')}</Button>
          </div>
        </div>
      </div>
    </div>
