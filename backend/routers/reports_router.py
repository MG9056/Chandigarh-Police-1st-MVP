...
Provides secured endpoints for generating, listing, and retrieving
intelligence reports for an investigation.

Endpoints:
- POST /api/investigations/{investigation_id}/reports
  Generates a grounded intelligence report.
  Requires: Authentication, Recent Re-Authentication (10m), v2 Investigation Modification Access.
...
def create_report(
    ...
    3. Investigation must exist (404 if not found).
    4. User must have modification access to this investigation (403 if unauthorized).
    5. Investigation must have reviewed/relevant findings or promoted evidence (400 if empty).
    6. Generates grounded analysis using AIService without hallucinating ungrounded facts.
    ...
    if not check_investigation_modification_access_v2(current_user, investigation, db):
        raise HTTPException(
            ...
            detail="You do not have permission to generate reports for this investigation.",
        )
    ...
    try:
        ...
        # Audit log failure with sanitized error description
        error_summary = "AI service unavailable" if http_err.status_code == 503 else "Validation or processing error"
        create_audit_log(
            db=db,
...
def list_reports(
    ...
    reports = (
        db.query(Report)
        .filter(Report.investigation_id == investigation.investigation_id)
...
def get_report(
    ...
    """
    Retrieves full details and grounded evidence mappings for a specific report.
    Requires broad READ permission.
    """
    investigation = (
        db.query(Investigation)
    ...
    report = (
        ...
        .filter(
            ...
            Report.investigation_id == investigation.investigation_id,
        )
        .first()
    )
    if not report:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Report '{report_id}' not found for investigation '{investigation_id}'.",
        )
