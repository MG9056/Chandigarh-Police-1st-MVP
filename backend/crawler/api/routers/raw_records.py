...
router = APIRouter(
    prefix="/api/raw-records",
    tags=["Raw Records Output Contract"],
)


@router.get("")
def list_raw_records(
    relevance: Optional[str] = Query(default="all"),
    ...
    # Global Intelligence relevance filter
    if relevance == "relevant":
        query = query.filter(
            RawRecord.relevance_label == "relevant"
        )
        ...
        "items": [
            {
                ...
                "relevance_confidence": (
                    float(r.relevance_confidence)
                    if r.relevance_confidence is not None
                    else None
                ),
                ...
                # Keep status in the response because other parts
                # of the pipeline may still use it.
                "status": r.status,
