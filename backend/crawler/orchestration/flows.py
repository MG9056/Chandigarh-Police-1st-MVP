...
async def run_crawl(
    ...
    """
    Executes an end-to-end crawler pipeline flow parameterized by (source_id, case_id).
    Coordinates collectors, policies, deduplication, filtering, AI classification, entity extraction,
    ...
    run = CrawlerRun(
        id=uuid.uuid4(),
        source_id=uuid.UUID(str(source_id)) if isinstance(source_id, str) else source_id,
        case_id=case_id,
        status="RUNNING",
    ...
    try:
        ...
        if not source:
            run.status = "FAILED"
            run.error_summary = f"Source with ID {source_id} not found."
            run.finished_at = datetime.now(timezone.utc)
            db.commit()
            return run

        active_keywords = KeywordService.get_active_keywords(db, case_id=case_id)
        source_config = dict(source.config or {})
        source_config["keywords"] = active_keywords

        # 3. Instantiate collector & transport
        collector = CollectorRegistry.get_collector(source.source_type)
        ...
        for rec in fetched_records:
            ...
            try:
                rec_tagged = EvidenceTagger.tag_and_validate(rec, run_id=run.id)
            ...
            # Deduplication
            if Deduplicator.is_duplicate(content_hash, db):
                logger.info(f"Duplicate content hash {content_hash} for {url}, skipping.")
                continue

            # Pipeline processing
            raw_text = rec_tagged.get("raw_text", "")
            cleaned_text = ContentCleaner.clean(raw_text)
            lang = LanguageDetector.detect(cleaned_text)

            # Keyword Pre-filter (cheap pass)
            matched_kws, is_passed = KeywordMatcher.match(cleaned_text, active_keywords)
            if not is_passed:
                ...
                raw_rec = RawRecord(
                    ...
                    relevance_reasoning="No active keywords matched.",
            ...
            rel_result = await classifier.classify(cleaned_text, matched_kws)
            ...
            elif rel_result.confidence < 0.8 or rel_result.label == "medical_legitimate":
        ...
        run.finished_at = datetime.now(timezone.utc)
        db.commit()
        logger.error(f"Crawler run failed: {exc}", exc_info=True)
